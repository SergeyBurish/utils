...
String isolHandleTariffsFile(String handleExcelJson) {
  ...
  try {
    ...
    final LamodaTariffs lamodaTariffs = <DateTime, Tariffs>{};
    for (int column = trStartColumn; ; column += 3) {
      ...
      lamodaTariffs[dateTime] = _getTariffs(sheet, column);
    }
    if (lamodaTariffs.isEmpty) {
      return _outputJson(error: 'no_data_found');
    }
    return _outputJson(lamodaTariffs: lamodaTariffs);
